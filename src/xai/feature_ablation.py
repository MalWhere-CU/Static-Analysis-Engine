...
class FeatureAblation(BaseXAI):
    """Feature Ablation on the extracted MLP via Captum.

    Replaces each channel with zero (baseline) one at a time and measures
    prediction change. Gives exact, model-agnostic causal attributions.
    """
    ...
    def __init__(self, model, window_size: int = 256, top_k: int = 10):
        ...
        self.fa = CaptumFeatureAblation(self.mlp_model)
    ...
    def attribute(self, file_bytes: bytes, target_class: Optional[int] = None) -> AttributionResult:
        if len(file_bytes) == 0:
            return AttributionResult(
                file_path="", method="feature_ablation",
        ...
        per_byte_scores = self._compute_ablation(post_conv, winner_indices, target_class, n_bytes)
    ...
    def _compute_ablation(self, post_conv: torch.Tensor, winner_indices: np.ndarray,
                          target_class: int, n_bytes: int) -> np.ndarray:
        """Apply Captum FeatureAblation on MLP and map to bytes."""
        post_conv_input = post_conv.detach()

        # Baseline: zeros (no activation)
        baselines = torch.zeros_like(post_conv_input)

        # Each feature (channel) is ablated individually
        # feature_mask not needed since input is 1D (each position is a feature)
