...
class SmoothGradXAI(BaseXAI):
    """SmoothGrad attribution using Captum on the extracted MLP.

    Strategy:
    1. Forward pass through full model → get post_conv features + winner indices
    2. Apply Captum NoiseTunnel(Saliency) on MLP (post_conv → logits)
    ...
    def __init__(self, model, window_size: int = 256, top_k: int = 10,
                 n_samples: int = 50, stdevs: float = 0.1):
        ...
        saliency = Saliency(self.mlp_model)
        self.smooth_grad = NoiseTunnel(saliency)
    ...
    def attribute(self, file_bytes: bytes, target_class: Optional[int] = None) -> AttributionResult:
        ...
        # Apply SmoothGrad on the MLP
        post_conv_attr = post_conv.detach().requires_grad_(True)

        attr = self.smooth_grad.attribute(
            post_conv_attr,
            nt_type='smoothgrad',
