...
class GradientSHAP(BaseXAI):
    """GradientSHAP on the extracted MLP via Captum.

    Uses multiple random baselines (sampled from a Gaussian centered at zero)
    to compute SHAP-like attributions that are less sensitive to baseline choice.
    """

    def __init__(self, model, window_size: int = 256, top_k: int = 10,
                 n_samples: int = 50, stdevs: float = 0.0001):
        """
        Args:
            ...
            stdevs: Standard deviation of noise added to each input sample.
        """
        ...
        self.gs = CaptumGradientShap(self.mlp_model)
    ...
    def attribute(self, file_bytes: bytes, target_class: Optional[int] = None) -> AttributionResult:
        if len(file_bytes) == 0:
            return AttributionResult(
                file_path="", method="gradient_shap",
        ...
        per_byte_scores = self._compute_shap(post_conv, winner_indices, target_class, n_bytes)
    ...
    def _compute_shap(self, post_conv: torch.Tensor, winner_indices: np.ndarray,
                      target_class: int, n_bytes: int) -> np.ndarray:
        """Apply Captum GradientSHAP on MLP and map to bytes."""
        post_conv_attr = post_conv.detach().requires_grad_(True)

        # Create baseline distribution: multiple random samples around zero
        # This gives SHAP values that account for different "absence" scenarios
        baselines = torch.randn(self.n_samples, post_conv_attr.shape[1]) * 0.01
